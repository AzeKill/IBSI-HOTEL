...
try {
    ...
    //Recipients
    $mail->setFrom('contact@ibsi.site', 'Contact IBSI');          //This is the email your form sends From
    ...
    Réponse:<br>
    $_GET[reponse]
    <br>L'équipe du IBSI Hotel.<br>
    <img src='http://ibsi.site/image/logo.png' alt='logo'/>";

    $mail->send();
    $sql ="DELETE FROM contact where idContact = $_GET[idReponse]";
    $stmt = $pdo->query($sql);
    echo "<script type='text/javascript'> window.location='../messagesContact.php'</script>";
} catch (Exception $e) {
